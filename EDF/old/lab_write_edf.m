...
maxV = max(data, [], 2);
minV = min(data, [], 2);
if max(maxV) > 32767 || min(minV) < -32767
...
% [tmp,monthstr] = month([num2str(header.day,'%02i') '-' num2str(header.month,'%02i') '-' num2str(header.year)],'dd-mm-yyyy');
clearvars tmp
monthstr = upper(header.month);
if header.year > 2000
...
if ~isfield(header,'technician')
    header.technician = 'X';
end
if ~isfield(header,'equipment')
    header.equipment = 'X';
end
if ~isfield(header,'subject')
    header.subject = [];
end
if ~isfield(header.subject,'ID')
...
if ~isfield(header.subject,'year')
    ...
    [tmp,tmp2] = month([num2str(header.subject.day,'%02i') '-' num2str(header.subject.month,'%02i') '-' num2str(header.subject.year,'%04i')],'dd-mm-yyyy');
    tmp2 = upper(tmp2);
